...
class CharacterMotorMovement {
	...
	var clampSpeedOnSlopes : boolean = false;
	var maxSpeedOnSlope : AnimationCurve = AnimationCurve(Keyframe(-90, 10), Keyframe(0, 10), Keyframe(90, 0));
...
function FixedUpdate () {
	if(grounded)
	{
    	if(GameManager.showMenu) return;
	}
...
function OnControllerColliderHit (hit : ControllerColliderHit) {
	...
	var normal = (hit.collider.GetType() == BoxCollider ? Vector3.up : hit.normal);
