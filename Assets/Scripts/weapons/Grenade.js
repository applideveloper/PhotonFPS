...
class Grenade extends MonoBehaviour
{
	...
	function Detonate ()
	{
        ...
        if(Local)
        {
		    ...
		    var distanceremote : float = Vector3.Distance( Camera.main.transform.position, _explosionPosition);
		    soldierCamera = Camera.main.gameObject.GetComponent(SoldierCamera);
		    soldierCamera.StartShake(distanceremote);
		}
		var body : Rigidbody;
		...
		if(col != null)
		{
			for(var c : int = 0; c < col.length; c++)
			{
				...
				if(Local)
				{
				    col[c].gameObject.BroadcastMessage("HitSoldierGranade","Player", SendMessageOptions.DontRequireReceiver);
				}
				body = null;
				body = col[c].gameObject.rigidbody;
                
                if(col[c].gameObject.tag != "EnemyPlayer" && col[c].gameObject.layer != 13)
				{
				    //Debug.Log(col[c].gameObject.tag+" __ "+col[c].gameObject.layer);
